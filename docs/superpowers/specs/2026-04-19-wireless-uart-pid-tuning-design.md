...
无线串口协议定义 3 个应用层状态。以下状态只作用于无线串口 PID 协议，不影响 `imu_test()` 的烧录器虚拟串口输出：

- `STREAMING`：持续发送无线串口数据
...
5. 若工程中存在独立的无线串口连续发送入口，则通过统一开关判断是否发送；`imu_test()` 不接入该开关，继续走烧录器虚拟串口

## 9. 无线串口数据流控制策略

### 9.1 无线串口发送门控

本次协议只控制无线串口 PID 相关的数据流与命令回包，不控制 `imu_test()` 的烧录器虚拟串口调试输出。

本次需要把无线串口数据发送改成“受开关控制”的行为：

- 允许发送时，保持无线串口原有数据流行为
- 暂停发送时，无线串口停止连续数据发送，仅保留命令回包
