...
当前工程需要增加一套稳定的无线串口 PID 查询与调参协议，用于在线查看和修改平衡车的三组 PID 参数。当前工程中还存在通过烧录器虚拟串口进行的 `imu_test()` 调试输出，但该路径不属于本次无线串口 PID 功能范围。
...
- 任何进入查询或调参流程的命令都要先暂停无线串口数据流
- 仅在收到 `start` 后恢复无线串口数据流，收到 `stop` 时随时暂停
...
- 当前 `code/app/imu_app.c` 中存在 `imu_test()` 调试输出路径，但该路径继续用于烧录器虚拟串口测试，不纳入本次无线串口 PID 协议控制
...
- 修改 `code/app/balance_app.c`，让 `pid_test()` 的无线数据流受协议开关控制
- 修改 `code/app/schedule.c`，把 PID 数据流与命令处理都挂到测试调度器

本次不修改：

- `code/driver/pid/*` 的 PID 计算公式
- `libraries/zf_device/*` 或其他底层无线串口驱动
- PIT 中断里的平衡控制主算法与 PID 调用链
- `code/app/imu_app.c` 与 `imu_test()` 的烧录器虚拟串口调试路径
...
2. 在 `uart_app` 内建立统一的串口命令状态机，并集中管理“无线串口数据流开关 + PID 参数映射 + 命令解析”
...
- 无线串口会话的暂停/恢复、本次正在等待哪组 PID 参数、字符串解析等状态应统一管理
- `balance_app` 只暴露 PID 实例，不承担串口协议职责，边界更清晰
- 底层驱动应保持纯粹，避免把命令语义耦合到设备驱动层
- `pid_test()` 本来就是测试链路的一部分，本次只需要让它受协议开关控制，不需要把它迁出调度器
...
回包不要求与其他调试串口输出同格式，只要求可人工辨认、便于串口助手查看。

## 6. 状态机设计

### 6.1 运行状态

无线串口协议定义 3 个应用层状态。以下状态只作用于无线串口 PID 协议与 `pid_test()` 的无线测试数据流，不影响 `imu_test()` 的烧录器虚拟串口输出：

- `STREAMING`：持续发送 `pid_test()` 的无线串口测试数据
- `PAUSED`：停止无线串口数据发送，仅接收命令
...
- `STREAMING` 收到任意 `check-*` 后，先停止无线串口数据流，再返回当前 PID 参数，并进入 `PAUSED`
- `STREAMING` 收到任意 `change-*` 后，先停止无线串口数据流，再进入 `WAITING_PARAM`
...
- 在 `WAITING_PARAM` 中收到 `stop`，应立即停止无线串口数据流并保持/回到非发送状态
- 在 `WAITING_PARAM` 中收到 `start`，应清除当前等待修改上下文并恢复无线串口数据流
...
5. 由 `schedule.c` 周期调用该命令入口；UART2 接收中断仍只负责把字节写入底层 FIFO，不在 ISR 内做协议解析
6. 在 `balance_app.c` 的 `pid_test()` 中通过统一开关判断是否发送 `JustFloat_Test_five(...)`
7. `imu_test()` 不接入该开关，继续走烧录器虚拟串口

## 9. 无线串口数据流控制策略

### 9.1 无线串口发送门控

本次协议只控制无线串口 PID 相关的数据流开关与命令回包，不控制 `imu_test()` 的烧录器虚拟串口调试输出。

本次需要把无线串口数据发送改成“受开关控制”的行为：

- 允许发送时，`pid_test()` 保持原有 `JustFloat_Test_five(...)` 发送行为
- 暂停发送时，`pid_test()` 停止连续无线数据发送，仅保留命令回包
- `imu_test()` 路径保持不变，不做门控改造
...
查询完成和调参完成后都不自动恢复无线串口数据流。

只有在收到 `start` 后才恢复无线串口数据发送。这样更符合调参场景，用户可以连续执行：
...
- 默认 `pid_test()` 无线数据流持续发送
- 输入 `stop` 后，无线串口数据流停止发送
- 输入 `start` 后，无线串口数据流恢复发送
- 输入 `check-steering_pid` 后，无线串口数据流停止且返回 `steering_pid` 的 5 参数
- 输入 `change-angle_pid` 后，无线串口数据流停止且进入等待参数状态
...
- 在 `WAITING_PARAM` 中输入 `start`，应恢复无线串口数据流并退出等待状态
...
- 调参完成后没有自动恢复无线串口数据流
- 协议解析放进 `schedule.c` 挂接的测试任务，由 UART2 RX 中断先写 FIFO，再由调度任务解析
