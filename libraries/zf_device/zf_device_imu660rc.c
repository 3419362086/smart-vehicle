...
#define IMU660RC_BIAS_MAX_SAMPLES   1024
...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     float转半精度浮点(fp16)
//-------------------------------------------------------------------------------------------------------------------
static uint16 float_to_fp16(float f)
{
    uint32 fbits = *(uint32 *)&f;
    uint16 h_sgn = (uint16)((fbits >> 16) & 0x8000u);
    uint32 f_exp = (fbits & 0x7F800000u);
    uint32 f_sig = (fbits & 0x007FFFFFu);

    if(f_exp >= 0x47800000u)
    {
        return (uint16)(h_sgn + 0x7C00u);
    }
    ...
    return (uint16)(h_sgn + ((f_exp - 0x38000000u) >> 13) + (f_sig >> 13));
}

//-------------------------------------------------------------------------------------------------------------------
// 函数简介     int16 排序（升序，用于 trimmed mean）
//-------------------------------------------------------------------------------------------------------------------
static void sort_int16(int16 *arr, uint16 n)
{
    uint16 i, j;
    int16 tmp;
    for(i = 1; i < n; i++)
...
//-------------------------------------------------------------------------------------------------------------------
// 函数简介     将 gbias(dps) 通过 ST 官方流程注入 SFLP
// 参数说明     bias_dps    三轴零偏 单位°/s
// 备注信息     严格按照 AN5763 Section 6.5.1 的 13 步流程
//-------------------------------------------------------------------------------------------------------------------
static void imu660rc_inject_gbias(float bias_dps[3])
{
    uint8  conf_saved[2];
    uint8  emb_func_en_saved[2];
    uint8  ctrl10_val;
    uint8  ctrl8_val;
    uint8  acc_fs_shift;
    uint8  status;
    ...
    // k因子：480Hz=0.00125
    float k = 0.00125f;
    switch(imu660rc_quarternion_rate)
    {
        case IMU660RC_QUARTERNION_15HZ:  k = 0.04f;    break;
        case IMU660RC_QUARTERNION_30HZ:  k = 0.02f;    break;
        case IMU660RC_QUARTERNION_60HZ:  k = 0.01f;    break;
        case IMU660RC_QUARTERNION_120HZ: k = 0.005f;   break;
        case IMU660RC_QUARTERNION_240HZ: k = 0.0025f;  break;
        case IMU660RC_QUARTERNION_480HZ: k = 0.00125f; break;
        default: break;
    }

    // Step 1: 转换 bias: dps -> rad/s -> 除以k -> fp16
    gbias_hf[0] = float_to_fp16(bias_dps[0] * (M_PI / 180.0f) / k);
    gbias_hf[1] = float_to_fp16(bias_dps[1] * (M_PI / 180.0f) / k);
    gbias_hf[2] = float_to_fp16(bias_dps[2] * (M_PI / 180.0f) / k);

    // Step 2: 保存 CTRL1/CTRL2，设置高性能模式
    conf_saved[0] = imu660rc_read_register(IMU660RC_CTRL1);
    conf_saved[1] = imu660rc_read_register(IMU660RC_CTRL2);
    imu660rc_write_register(IMU660RC_CTRL1, (conf_saved[0] & 0x0F) | 0x00);  // 保留ODR，OP_MODE=000 高性能
    imu660rc_write_register(IMU660RC_CTRL2, (conf_saved[1] & 0x0F) | 0x00);  // 保留ODR，OP_MODE=000 高性能

    // Step 4: 关闭嵌入式功能
    imu660rc_set_mem_bank(IMU660RC_EMBED_MEM_BANK);
    emb_func_en_saved[0] = imu660rc_read_register(IMU660RC_EMB_FUNC_EN_A);
    emb_func_en_saved[1] = imu660rc_read_register(IMU660RC_EMB_FUNC_EN_B);
    imu660rc_write_register(IMU660RC_EMB_FUNC_EN_A, 0x00);
    imu660rc_write_register(IMU660RC_EMB_FUNC_EN_B, 0x00);

    // Step 5: 等待 EMB_FUNC_ENDOP = 1
    do {
        status = imu660rc_read_register(IMU660RC_EMB_FUNC_EXEC_STAT);
    } while((status & 0x01) == 0);
    imu660rc_set_mem_bank(IMU660RC_MAIN_MEM_BANK);

    // Step 6: EMB_FUNC_DEBUG = 1 (CTRL10 bit6)
    ctrl10_val = imu660rc_read_register(IMU660RC_CTRL10);
    imu660rc_write_register(IMU660RC_CTRL10, ctrl10_val | 0x40);

    // Step 7: 重新使能 SFLP_GAME_EN
    imu660rc_set_mem_bank(IMU660RC_EMBED_MEM_BANK);
    emb_func_en_saved[0] |= 0x02;  // 强制 SFLP_GAME_EN = 1
    imu660rc_write_register(IMU660RC_EMB_FUNC_EN_A, emb_func_en_saved[0]);
    imu660rc_write_register(IMU660RC_EMB_FUNC_EN_B, emb_func_en_saved[1]);
    imu660rc_set_mem_bank(IMU660RC_MAIN_MEM_BANK);

    // Step 8: 读取加速度计数据，写入 sensor hub 寄存器
    // 等待加速度计数据就绪
    do {
        status = imu660rc_read_register(IMU660RC_STATUS_REG);
    } while((status & 0x01) == 0);  // XLDA bit

    imu660rc_read_registers(IMU660RC_OUTX_L_A, (uint8 *)xl_data, 6);
    ctrl8_val = imu660rc_read_register(IMU660RC_CTRL8);
    acc_fs_shift = ctrl8_val & 0x03;

    imu660rc_set_mem_bank(IMU660RC_HUB_MEM_BANK);  // 进入 sensor hub bank
    for(i = 0; i < 3; i++)
    {
        data_tmp = (int32)xl_data[i];
        data_tmp <<= acc_fs_shift;                 // 按当前 FS_XL 左移
        imu660rc_write_register(IMU660RC_SENSOR_HUB_1 + 3 * i + 0, (uint8)(data_tmp & 0xFF));
        imu660rc_write_register(IMU660RC_SENSOR_HUB_1 + 3 * i + 1, (uint8)((data_tmp >> 8) & 0xFF));
        imu660rc_write_register(IMU660RC_SENSOR_HUB_1 + 3 * i + 2, (uint8)((data_tmp >> 16) & 0xFF));
    }
    // SENSOR_HUB_10~18 清零
    for(i = 0; i < 9; i++)
    {
        imu660rc_write_register(0x0B + i, 0x00);  // HUB_10=0x0B
    }
    imu660rc_set_mem_bank(IMU660RC_MAIN_MEM_BANK);

    // Step 9: 等待至少30us，然后等 EMB_FUNC_ENDOP
    system_delay_ms(1);
    imu660rc_set_mem_bank(IMU660RC_EMBED_MEM_BANK);
    do {
        status = imu660rc_read_register(IMU660RC_EMB_FUNC_EXEC_STAT);
    ...
    // Step 10: 通过 PAGE 机制写入 gbias 到嵌入式高级特性寄存器
    imu660rc_set_mem_bank(IMU660RC_EMBED_MEM_BANK);
    // 设置写模式
    imu660rc_write_register(IMU660RC_PAGE_RW, 0x40);        // PAGE_WRITE = 1
    // 选择 page 0，设置 not_used0 = 1（默认值）
    imu660rc_write_register(IMU660RC_PAGE_SEL, 0x01);       // page_sel=0, not_used0=1
    // ST 官方驱动使用连续 PAGE_VALUE 写入，不重复设置 PAGE_ADDRESS
    imu660rc_write_register(IMU660RC_PAGE_ADDRESS, IMU660RC_SFLP_GBIASX_L);
    for(i = 0; i < 6; i++)
    {
        imu660rc_write_register(IMU660RC_PAGE_VALUE, ((uint8 *)gbias_hf)[i]);
    }
    imu660rc_write_register(IMU660RC_PAGE_SEL, 0x01);
    imu660rc_write_register(IMU660RC_PAGE_RW, 0x00);         // 关闭写模式
    imu660rc_set_mem_bank(IMU660RC_MAIN_MEM_BANK);

    // Step 11: 恢复传感器配置（保留 SFLP_GAME_EN）
    imu660rc_write_register(IMU660RC_CTRL1, conf_saved[0]);
    imu660rc_write_register(IMU660RC_CTRL2, conf_saved[1]);

    // Step 12: 关闭 EMB_FUNC_DEBUG
    imu660rc_write_register(IMU660RC_CTRL10, ctrl10_val & ~0x40);
}

//-------------------------------------------------------------------------------------------------------------------
// 函数简介     IMU660RC 零偏校准（静止采样 + trimmed mean + 注入SFLP）
// 参数说明     duration_ms     采样时长（毫秒），建议 2000
// 返回参数     uint8           0-成功  1-失败
// 使用示例     imu660rc_bias_calibrate(2000);
// 备注信息     调用前确保传感器已初始化且设备静止
//-------------------------------------------------------------------------------------------------------------------
uint8 imu660rc_bias_calibrate(uint32 duration_ms)
{
    static int16 gx_buf[IMU660RC_BIAS_MAX_SAMPLES];
    static int16 gy_buf[IMU660RC_BIAS_MAX_SAMPLES];
    static int16 gz_buf[IMU660RC_BIAS_MAX_SAMPLES];
    uint16 cnt = 0;
    uint32 t_start;
    int16  gyro_raw[3];
    uint16 trim, i;
    ...
    t_start = system_getval_ms();
    while((system_getval_ms() - t_start) < duration_ms && cnt < IMU660RC_BIAS_MAX_SAMPLES)
    {
        if(imu660rc_read_register(IMU660RC_STATUS_REG) & 0x02)
        {
            imu660rc_read_registers(IMU660RC_OUTX_L_G, (uint8 *)gyro_raw, 6);
            gx_buf[cnt] = gyro_raw[0];
            gy_buf[cnt] = gyro_raw[1];
            gz_buf[cnt] = gyro_raw[2];
            cnt++;
        }
    }

    sort_int16(gx_buf, cnt);
    sort_int16(gy_buf, cnt);
    sort_int16(gz_buf, cnt);
    ...
    bias_dps[0] = ((float)sum_x / (float)(cnt - 2 * trim)) / imu660rc_transition_factor[1];
    bias_dps[1] = ((float)sum_y / (float)(cnt - 2 * trim)) / imu660rc_transition_factor[1];
    bias_dps[2] = ((float)sum_z / (float)(cnt - 2 * trim)) / imu660rc_transition_factor[1];

    imu660rc_inject_gbias(bias_dps);
...
uint8 imu660rc_init(imu660rc_quarternion_rate_config quarternion_rate)
{
    ...
    do
    {
        ...
        if(IMU660RC_QUARTERNION_DISABLE != quarternion_rate)
        {
            ...
            imu660rc_write_register(IMU660RC_EMB_FUNC_INIT_A, 0x02);
            imu660rc_write_register(IMU660RC_PAGE_RW, 0x00);
            imu660rc_set_mem_bank(IMU660RC_MAIN_MEM_BANK);

            // 开启中断检测引脚
            exti_init(ERU_CH3_REQ3_P10_3, EXTI_TRIGGER_RISING);
        }
    }while(0);
