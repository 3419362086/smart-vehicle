...
void wireless_uart_pid_service(void)
{
    ...
    const PID_T *pid = &steering_pid;
    uint32 line_text_length = (uint32)strlen(uart_pid_line_buffer);

    (void)parse_float;
    (void)pid;
    (void)uart_pid_state;
    (void)uart_pid_pending_target;

    if (uart_pid_line_length > line_text_length)
    {
