...
* 许可申明英文版 able able able able able
* 许可证随附 able able able able able 文件
...
IFX_INTERRUPT(exti_ch1_ch5_isr, 0, EXTI_CH1_CH5_INT_PRIO)
{
    ...
    if(exti_flag_get(ERU_CH1_REQ10_P14_3))          // 通道1中断
    {
        ...
        tof_module_exti_handler();                  // ToF 模块 INT 引脚外部中断
...
// 摄像头pclk引脚默认占用了 2通道，因为在触发DMA所以这里不再定义中断函数
...
IFX_INTERRUPT(exti_ch3_ch7_isr, 0, EXTI_CH3_CH7_INT_PRIO)
{
    ...
    if(exti_flag_get(ERU_CH3_REQ3_P10_3))           // 通道3中断
    {
        ...
        imu660rc_callback();                        // IMU660RC INT2 中断回调
    }
//    if(exti_flag_get(ERU_CH3_REQ6_P02_0))           // 摄像头通道3中断
//    {
//        exti_flag_clear(ERU_CH3_REQ6_P02_0);
//        camera_vsync_handler();                     // 摄像头场信号采集回调
//    }
...
IFX_INTERRUPT(uart0_rx_isr, 0, UART0_RX_INT_PRIO)
{
    interrupt_global_enable(0);                     // 开启中断嵌套
        ...
        debug_interrupr_handler();                  // 调用 debug 串口接收处理函数 数据会被 debug 串口缓冲区读取
#endif                                              // 如果修改了 DEBUG_UART_INDEX 宏定义则需要放到对应的串口中断去
}
...
{
    interrupt_global_enable(0);                     // 开启中断嵌套
//    camera_uart_handler();                          // 摄像头串口命令统一回调函数
    uart_control_callback();                          // FOC控制串口接收回调
}
